...
setInterval(() => {
  http.get(`http://ghostv1.glitch.me/`); /// حط اسم المشروع تبعك name تعديل مهم بدل
}, 280000);
////بكجات
...
client.on("ready", () => {
  ...
  client.user.setActivity(`DarkTown RolePlay Server || سيرفر البلدة المظلمة للحياة الواقعية`, { type: "PLAYING" });
});
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@725865224325955594>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("message", message => {
  if (message.content === "السلام عليكم") {
    message.reply("**:heart:وعليكم السلام ورحمة الله وبركاته:heart:**");
  ...
  console.log(`Logged in as ${client.user.tag}!`);
         ...
         client.on('message', message => {
            if (message.content === 'رابط') {
              message.reply('**https://discord.gg/WaXXAqA**');
...
client.on('ready', () => {
  console.log(`Logged in as ${client.user.tag}!`);
         ...
         client.on('message', message => {
            ...
            message.reply  (`
**- Press F8**
**- Then type: [ connect cfx.re/join/oa9ybx ]**
**- Good Luck**`);
...
client.on('message', message => {
  if (message.channel.id === "724020007687684555") {
    message.react('<a:DT_true:724657569284751400')
      .then(() => {
        message.react('<a:DT_false:724657646442905691')
      });
  }
});
...
let args = message.content.split(' ');
  ...
  const currentSize = message.guild.members.filter(m => m.voiceChannel).size;
        ...
        .setFooter("© DarkTown, All Rights Reserved 2020")
...
client.on('message',  message => {
  if(bannedwords.some(word => message.content.includes(word))) {
    message.delete()
    message.reply(" احترم نفسك, يمنع الشتم بالشات  ").then(msg => {msg.delete(5000)});;
  };
});
        ...
        .then(collected => {
          ...
          message.channel
            ...
            .then(async collectedd => {
              ...
              let steve = await client.fetchUser("471059639124099072");
              embed1.setFooter(
                `رد تلقائي`,
...
client.on('message' , message => {
    ...
    client.login('NzI1ODY1MjI0MzI1OTU1NTk0.XvX0lg.F3GcMZcalItqq4JESL0ID-WvjSw')
